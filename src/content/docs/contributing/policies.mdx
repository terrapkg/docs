...
Ship GUI apps with [appstream metadata](/contributing/appstream) and desktop file.
...
- Submissions SHOULD be placed in the corresponding directory in `anda/`, linked [here](https://github.com/terrapkg/packages/tree/frawhide/anda).
  ...
  - The following directories have a higher priority. Categorize your new package in the following order:
    ...
    3. `buildsys/` (Buildsystems, software used to build other software)
    4. `devs/` (Tools for software development)
    5. `games/`
    6. `themes/`
    7. `desktops/` (Applications specific to, or relates to, a Desktop Environment/Window Manager)
    ...
    11. `tools/`
    12. `tools/ansible-collections` (Ansible collection packages MUST be placed here)
...
<Aside title="Note" icon="comment-alt">Our AppStream guidelines are documented [here](/contributing/appstream).</Aside>

All graphical applications that can be started by the user MUST provide a [desktop entry](/contributing/guidelines#providing-your-own-desktop-file) and [AppStream MetaInfo](https://www.freedesktop.org/software/appstream/docs/chap-Quickstart.html#qsr-app-example) file. Other applications are also RECOMMENDED to do so.
