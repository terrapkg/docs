...
- Submissions SHOULD be placed in the corresponding directory in `anda/`, linked [here](https://github.com/terrapkg/packages/tree/frawhide/anda).
  ...
  - The following directories have a higher priority. Categorize your new package in the following order:
    ...
    9. `desktops/stardust/` (packages for the [Stardust XR[(https://stardustxr.org/) stack)
    10. `apps/` (GUI applications)
    11. `langs/` (software written in/for a specific programming language)
...
`%unreleased_version{:rpmspec}` refers to the expected version number for the next stable (or a more stable tag than `nightly{:rpmspec}`) release,
