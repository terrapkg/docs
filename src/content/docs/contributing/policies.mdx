...
If upstream does not provide a version, use `Commit tacking (no stable version tag)` and replace <Code code={`%unreleased_version`} lang={RPMSpec} inline /> as <Code code={`0`} lang={RPMSpec} inline />.
