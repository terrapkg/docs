...
Before creating your `metainfo.xml` file, check out our [AppStream helper macro](../appstream#terra_appstream), it can save a lot of effort.
...
<Aside type="caution">
  You MUST NOT use the <Code code={`<developer_name/>`} lang="xml" inline /> tag.
</Aside>
...
<Code code={`%terra_appstream`} lang={RPMSpec} inline /> will add more sections to your `metainfo.xml`, or can generate your metainfo.xml file entirely.
You SHOULD use it when installing your own metainfo.xml file.
Review the [Terra AppStream helper README](https://github.com/terrapkg/appstream-helper/blob/main/README.md) for more.
You MAY need to add this line to your spec file: <Code code={`BuildRequires: terra-appstream-helper`} lang={RPMSpec} inline />

<Code code={`%appid`} lang={RPMSpec} inline /> SHOULD be defined at the top of your spec file (preamble), and used in references to the metainfo file name, for example:
