...
If you have difficulty populating the `.metainfo.xml` file, hop into our [Discord](https://fyralabs.com/discord) server, or make a draft pull request asking for assistance.
...
- Graphical apps
- TUI applications that make sense to be opened in a graphical environment (ships a `.desktop` file)

Other packages MAY also provide appstream metadata files.

## Creating the `metainfo.xml` File

Before creating your `metainfo.xml` file, check out our [AppStream helper macro](#terra_appstream), it can save a lot of effort.

Determine your package's app ID, then start with the template below.
The metainfo file MUST be named <Code code={`[1;3mapp_id[0m.metainfo.xml`} lang="ansi" inline />.
...
- If your package is only hosted on GitHub (no website, not a GitHub mirror), then the app ID MUST be <Code code={`com.github.[1;3mAPP-NAME`} lang="ansi" inline />.
...
Other tags exist that are not listed, you are free to use them, but they're generally not useful.
...
The <Code code={`<keywords/>`} lang="xml" inline /> tag[^1] will make your package easier find when users search these terms:
...
This tag[^2] is to specify the developer/s of your package. This can be a person or group/organization.

<Aside type="caution">
  You MUST NOT use the <Code code={`<developer_name/>`} lang="xml" inline />{" "}
  tag.
</Aside>

#### Screenshots

This tag[^3] can be either a video or a picture, and is used to showcase the package in software stores.
...
## Using your metainfo.xml file in your .spec file

### Macros

#### %terra_appstream

<Code code={`%terra_appstream`} lang={RPMSpec} inline /> will add more sections
to your `metainfo.xml`, or can generate your metainfo.xml file entirely. You
SHOULD use it when installing your own metainfo.xml file. Review the [Terra
AppStream helper
README](https://github.com/terrapkg/appstream-helper/blob/main/README.md) for
more. You MAY need to add this line to your spec file:
<Code code={`BuildRequires: terra-appstream-helper`} lang={RPMSpec} inline />

<Code code={`%appid`} lang={RPMSpec} inline /> SHOULD be defined at the top of
your spec file (preamble), and used in references to the metainfo file name, for
example:
...
[^1]: https://www.freedesktop.org/software/appstream/docs/chap-Metadata.html#tag-keywords

[^2]: https://www.freedesktop.org/software/appstream/docs/chap-Metadata.html#tag-developer

[^3]: https://www.freedesktop.org/software/appstream/docs/chap-Metadata.html#tag-screenshots
