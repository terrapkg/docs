...
In most cases, <Code code={`rpm.version(gh("..."));`} lang="ts" inline /> as a one-liner is enough to make it work™.
Documentations for more functions are available in the [Andaman
devdocs](https://developer.fyralabs.com/andaman/andax#custom-functions).

Other functions are available in the `andax/` directory after <Code code={`import`} lang="ts" inline />ing.
...
<Aside type="caution">
  Avoid using the{" "}
  <span title="This includes gh() and gh_tag()">GitHub API</span> for fetching
  the latest versions. You may use GitLab, crates.io, Pypi, etc. instead. This
  is because GitHub has an API rate-limit.
</Aside>
<Aside>
  You might see scripts with `if filters.contains("nightly")`. This `if` statement is no longer needed.
</Aside>
<Aside type="caution">Avoid running code unrelated to updating/bumping packages.</Aside>
