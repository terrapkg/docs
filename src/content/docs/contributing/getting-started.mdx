...
description: "Get started in contributing packages to Terra."
---
...
<Tabs>
  <TabItem label="Fedora/Ultramarine">
    ...
    ```sh
    sudo dnf install anda mterra-mock-configs
    ```
  ...
  <TabItem label="Dev Container">
    ...
    - Install the Dev Containers extension in your IDE
    ...
    - Fork [terrapkg/packages](https://github.com/terrapkg/packages), and `git clone` your fork
    - Open the folder you cloned in your editor
    - It should prompt you to `reopen in Dev Container`, click this and wait for the Dev Container to open.
...
1. Inside the folder, create `anda.hcl` and `<packagename>.spec`.
   ...
   - For more information, check out our [packaging policies](/contributing/policies).
...
- Add <Code code={`Source0:`} lang={RPMSpec} inline /> or <Code code={`Source1:`} lang={RPMSpec} inline /> or more.
  These preambles should link to a compressed file (preferably a tarball) and will be extracted during <Code code={`%prep`} lang={RPMSpec} inline />.
  You can find this by looking at the GitHub releases page (usually a .tar.gz link).
- Add a new line <Code code={`%prep`} lang={RPMSpec} inline />
  - The source file will be automatically downloaded and
    extracted with <Code code={`%autosetup -n <root dir name in tar file>`} lang={RPMSpec} inline /> inside <Code code={`%prep`} lang={RPMSpec} inline />.
  - If it is not a tar archive, extract the file manually with a command.
- Inside <Code code={`%build`} lang={RPMSpec} inline />, you might need to build the package.
  Macros like <Code code={`%meson`} lang={RPMSpec} inline /> and <Code code={`%cmake`} lang={RPMSpec} inline /> are supported.
  - If not, manually state the command.
- List out all the files to be included inside <Code code={`%files`} lang={RPMSpec} inline />.
  - Use <Code code={`%doc`} lang={RPMSpec} inline /> to state the README file (if it exists).
  - Use <Code code={`%license`} lang={RPMSpec} inline /> to state the COPYING or LICENSE file (if it exists).
...
dependencies using <Code code={`[1;32msudo dnf[97m builddep [0;97;mpath/to/pkgname.spec`} lang="ansi" inline />!
...
- Review our [policies](/contributing/policies) and [guidelines](/contributing/guidelines)
...
The system regarding automatic updates [are described here](/contributing/autoupdate). Here is an overview:
...
- You also need to add <Code code={`labels { nightly = "1" }`} lang="hcl" inline /> in `anda.hcl` for nightly packages.

<Aside>Remember to add semicolons in Rhai scripts! (`;`)</Aside>
<Aside>
