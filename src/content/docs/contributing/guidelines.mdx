...
<Aside type="caution">
  REMEMBER TO [BUMP](#fixing-packages){" "}
  <Code code={`Release:`} lang={RPMSpec} inline /> if propose new changes
  (unless pkg with current release num doesn't exist in
  [repos](https://repos.fyralabs.com))
</Aside>
...
<CardGrid>
  <Card title="Incorrect:" icon="error">
    ```rpmspec %ifarch x86_64 %define arch %{nil}
    %endif %ifarch aarch64 %define arch arm64- %endif ```
  </Card>
  <Card title="Correct:" icon="approve-check-circle">
    ```rpmspec %ifarch x86_64 %define arch %{nil}
    %elifarch aarch64 %define arch arm64- %endif ```
  </Card>
</CardGrid>
