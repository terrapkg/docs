...
Available flags:
- `-n <…>` MUST be used to define `%{npm_name}{:rpmspec}` if it was not defined as a global macro.
...
Available flags:
- `-u <…>` MUST be used to define the URL if the one listed in the `URL:{:rpmspec}` tag for the package is not detectable as clonable.
...
Available flags:
- `-o <…>` MAY be used as a shorthand for `--out{:rpmspec}`.
...
It is REQUIRED to have `BuildRequires: nodejs-npm{:rpmspec}`.
...
- `-o <…>` will omit a dependency or group (such as `dev{:rpmspec}`) from being fixed.
...
It is REQUIRED to have `BuildRequires: bun-bin{:rpmspec}`.
...
It is REQUIRED to have `BuildRequires: deno{:rpmspec}`.
...
It is REQUIRED to have `BuildRequires: pnpm{:rpmspec}`.
...
You SHOULD use `BuildRequires: yarnpkg{:rpmspec}` (Yarn Classic) unless you need `BuildRequires: yarnpkg-berry{:rpmspec}` (Yarn Berry) specifically.
...
Generates `BuildRequires:{:rpmspec}` for a Tauri build.

Supports the same flags as [%tauri_buildrequires](#tauri_buildrequires).

<Aside type="caution">
  `BuildRequires:{:rpmspec}` generation only works in Mock.
</Aside>
<Aside>
  This does NOT install the JavaScript package managers used for builds except for NPM (e.g., `bun`, `deno`, `pnpm`, `yarnpkg`, `yarnpkg-berry`).<br />
  Specify these `BuildRequires:{:rpmspec}` manually.
...
- `-r <…>`: Executes `%{__npm} run{:rpmspec}` for scripts in the `package.json`. Can run multiple scripts if they are comma separated.
...
- `-r <…>`: Executes `%{__bun} run{:rpmspec}` for scripts in the `package.json`. Can accept multiple arguments if they are comma separated.
...
Available flags:
- `-f <…>`: This option is only for Tauri projects. Features to pass to Tauri/Cargo. Features should be comma separated.
- `-t <…>`: Executes `%{__deno} task{:rpmspec}` for scripts in the `package.json`. Can accept multiple arguments if they are comma separated.
- `-v{:rpmspec}`: Run Electron Vite build steps. Not needed for all Electron apps.
- `-r <…>`: Executes files with `%{__deno} run{:rpmspec}`. Can accept multiple arguments if they are comma separated.
...
- `-f <…>`: This option is only for Tauri projects. Features to pass to Tauri/Cargo. Features should be comma separated.
- `-r <…>`: Executes `%{__pnpm} run{:rpmspec}` for scripts in the `package.json{:rpmspec}`. Not recommended for Electron Builder/Electron Forge or Electron Vite commands. Can accept multiple arguments if they are comma separated.
- `-v{:rpmspec}`: Run Electron Vite build steps. Not needed for all Electron apps.
...
- `-f <…>`: This option is only for Tauri projects. Features to pass to Tauri/Cargo. Features should be comma separated.
- `-r <…>`: Executes `%{__yarn} run{:rpmspec}` for scripts in the `package.json`. Not recommended for Electron Builder, Electron Forge, or Electron Vite commands. Can accept multiple arguments if they are comma separated.
- `-v{:rpmspec}`: Run Electron Vite build steps. Not needed for all Electron apps.
