...
The metainfo file MUST be named <Code code={[1;3mapp_id[0m.metainfo.xml} lang="ansi" />.
...
- If your package is only hosted on GitHub (no website, not a GitHub mirror), then the app ID MUST be <Code code={com.github.[1;3mAPP-NAME} lang="ansi" />.
...
The <Code code={<keywords/>} lang="xml" /> tag[^2] will make your package easier find when users search these terms:
...
<Aside type="caution">
  You MUST NOT use the <Code code={<developer_name/>} lang="xml" /> tag.
</Aside>
...
<Code code={%terra_appstream} lang="rpmspec" /> will add more sections to your `metainfo.xml`, or can generate your metainfo.xml file entirely.
...
You MAY need to add this line to your spec file: <Code code={BuildRequires: terra-appstream-helper} lang="rpmspec" />

<Code code={%appid} lang="rpmspec" /> SHOULD be defined at the top of your spec file (preamble), and used in references to the metainfo file name, for example:
