...
Full support. Packages are updated on a rolling release basis by Terra’s autoupdate system.

## Non-latest, but supported versions of Fedora

Supported, as Terra is rolling, we provide updates through Terra’s autoupdate system. However, it is also not guaranteed that the packages are up to date as packages may have build issues on older branches. 
